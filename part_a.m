...
%large enough for all iterations
...
for e = 1:counter
    for i = 1:m
        for j = 1:n
            ...
            %upper nod of the tip
            elseif j == n
                if i == 1
                    T(i, j, e+1) = (((T(i+1, j, e) + T(i, j-1, e)) / 2) + (Bi * T_inf)) / (1 + Bi);
                
    
                %lower nod of the tip
                elseif i == m
                    T(i, j, e+1) = (((T(i-1, j, e) + T(i, j-1, e)) / 2) + (Bi * T_inf)) / (1 + Bi);
                
                %other nods of the tip
                else
                    T(i, j, e+1) = ((T(i, j-1, e) + (T(i+1, j , e) + T(i-1, j, e)) / 2) + Bi * T_inf) / (2 + Bi);
...
xlabel('Length (mm)')
ylabel('Thickness (mm)')
title('St-St conduction with convection in tip (part a)')
